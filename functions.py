...
def StimaProduzione(df):
  ...
  for index, row in df.iterrows(): 
    ...
    response = requests.get("https://re.jrc.ec.europa.eu/api/v5_1/PVcalc?lat="+str(row['Lat'])+"&lon="+str(row['Long'])+"&raddatabase=PVGIS-SARAH&peakpower="+str(row['Potenza Da Installare'])+"&pvtechchoice=crystSi&loss="+str(8)+"&optimalinclination=1&vertical_optimum=1&outputformat=json")
...
def CalcolaIncentivo(df, QuotaProCapite, Quota_Consumatori, Quota_Produttore, Quota_Prosumer, Quota_Idro): 
    ...
    if(df['Categoria'] == 'Consumer'): 
      ...
      #print('##########################################################################################')
      return QuotaProCapite + Quota_Consumatori*df['Percentuale Condivisa'] + Quota_Produttore*df['Percentuale Immessa'] + Quota_Prosumer*df['Percentuale Immessa Prosumer']
...
def simula(file, PUN, costo_medio_energia, incentivo_arera, incentivo_mise, ritiro_dedicato, val_prod_idro, inc_cond_idro, taglia_specifica, p_attribuibile, p_fissa, p_consumatori, p_produttore, Architettura_CER, Tempo_Ammortamento_Architettura, Commissione, Ammortamento_Impianto_Fotovoltaico, Tempo_Ammortamento_Fotovoltaico):
    p_costi_gestione = 1-p_attribuibile
    p_variabile = 1-p_fissa
    p_immissione = 1-p_consumatori
    p_prosumer = 1-p_produttore
    ...
    #########
    Quota_Fissa = (p_fissa*Incentivo_Attribuibile) #ATTENZIONE: QUESTA QUOTA E' DA DIVIDERE IN PARTI UGUALI FRA I MEMBRI DELLA COMUNITA'.
    ...
    df_finale['Incentivo Condivisione'] = df_finale.apply(CalcolaIncentivo, axis = 1, args = [QuotaProCapite, Quota_Consumatori, Quota_Produttore, Quota_Prosumer, Quota_Idro])
